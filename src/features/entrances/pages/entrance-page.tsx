...
import { EntranceFormDialog } from '../components/entrance-form-dialog'

// Hooks
import { useEntrancesQuery } from '../hooks/use-entrances-query'
import { useEntranceMutations } from '../hooks/use-entrance-mutations'
import { useEntranceHandlers } from '../hooks/use-entrance-handlers'

// Types
import type { EntranceListParams, EntranceStatusFilterValue } from '../types/entrances.types'

// Routes
import { entrancesPath } from '../routes/entrances.route'
...
const routeApi = getRouteApi('/_private/management/entrances')

/**
 * Página de gestão de portarias.
 *
 * Lista paginada com busca, filtro de status, criação/edição em dialog e
 * exclusão física com confirmação (409 bloqueia quando há dispositivos
 * vinculados via `device`). Acesso restrito a `MANAGE_ENTRANCES`.
 */
export function EntrancePage() {
  ...
  const canManage = useMemo(
    () => canAccess(user, { permissions: [PermissionCode.MANAGE_ENTRANCES] }),
    [user],
  )

  // --- Search params da rota ---
  const search = routeApi.useSearch()
  const listParams = useMemo<EntranceListParams>(
    () => ({
  ...
  // --- Queries ---
  const { data, isPending, error } = useEntrancesQuery(listParams)

  // --- Mutations ---
  const { createEntrance, updateEntrance, deleteEntrance } = useEntranceMutations()
  ...
  const isFormSubmitting = createEntrance.isPending || updateEntrance.isPending
  ...
  return (
    <PageLayout>
        ...
        filters={
          <>
            ...
            <div className="flex items-center gap-2">
              ...
              <Select
                value={statusValue}
                onValueChange={(value) => handleStatusChange(value as EntranceStatusFilterValue)}
              >
      ...
      {formState ? (
        <EntranceFormDialog
          key={formState.mode === 'edit' ? formState.entrance?.id : 'create'}
          open
      ...
      <ConfirmDialog
        ...
        variant="destructive"
        isPending={deleteEntrance.isPending}
      />
    </PageLayout>
  )
}
